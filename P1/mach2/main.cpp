...
void recursive_doubling_zeta(double *psum, double *sum_all){
    ...
    */
    ...
    for(int i = 0; i < log(numprocs); i++){
        ...
        MPI_Recv(&recbuff, 1, MPI_DOUBLE, sendId, 0, MPI_COMM_WORLD, &status);
            //Now the buffer should contain the partial sum from process sendId.
            //We noe add our sum + this as the new sum.
        *psum += recbuff;

            //printf("Process %d received %f all: %f\n", myid, recbuff, sum+recbuff);
    }
    //We need to put what is on the reciev buffer
    *sum_all = *psum;
}
